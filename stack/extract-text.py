...
def process_file(file_path):
    ...
    if os.path.exists(output_file):
        if os.path.getsize(output_file) > 50:  # Skip if output is bigger than 50 bytes
            print(f"[!] Skipping {file_path} (already converted and looks good).")
            return
        else:
            print(f"[!] Existing {output_file} is empty or too small, reprocessing...")
